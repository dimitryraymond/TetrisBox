...
class Tetris(object):
			...
			for j in range(16):
				for i in range(16):
					if(self.cubes[i][j][k] == 1):
						self.calculateColor(self.colors[i][j][k], .5, k)
						self.calculateCubeFaces([i, j, k])
	...
	def loadShadows(self):
		for i in range(len(self.floorFace)):
			self.drawQuadrilateral(self.rightWallFace[i])
	...
	def calculateCubeFaces(self, xyzPoint):
		x = xyzPoint[0]
		y = xyzPoint[1]
		z = xyzPoint[2]
		...
		#cube faces
		self.backFace = [i1, j1, i2, j1, i2, j2, i1, j2]
		self.frontFace = [x1, y1, x2, y1, x2, y2, x1, y2]
		self.rightFace = [x2, y1, i2, j1, i2, j2, x2, y2]
		self.leftFace = [x1, y1, i1, j1, i1, j2, x1, y2]
		self.topFace = [x1, y1, i1, j1, i2, j1, x2, y1]
		self.bottomFace = [x2, y2, i2, j2, i1, j2, x1, y2]

		#wall faces
		self.floorFace.append([x2, floorY2, i2, floorJ2, i1, floorJ2, x1, floorY2])
		self.ceilingFace = [x1, ceilingY1, i1, ceilingJ1, i2, ceilingJ1, x2, ceilingY1]
		self.rightWallFace.append([rightX2, y1, rightI2, j1, rightI2, j2, rightX2, y2])
		self.leftWallFace = [leftX1, y1, leftI1, j1, leftI1, j2, leftX1, y2]		
	...
	def drawCube(self, xyzPoint, rgb = (255, 0, 0)):
		...
		if(x <= 15):
			self.drawQuadrilateral(self.rightFace, self.rgb)
		elif(x >= 16):
			self.drawQuadrilateral(self.leftFace, self.rgb)
		else:
			print("couldnt draw right or left face")
			
		if(y <= 15):
			self.drawQuadrilateral(self.bottomFace, self.rgb)
		elif(y >= 16):
			self.drawQuadrilateral(self.topFace, self.rgb)
		else:
			print("Couldn't draw top or bottom faces")

		self.drawQuadrilateral(self.frontFace, self.rgb)

		"""if(x == 0 and y == 0 and z == 31):
			print(i1/self.getWidth())"""

	def drawShadow(self, rgb = (255, 0, 0)):
		pass
		
		
	def calculateColor(self, rgb, percentReduction, z = 0):
		#lower color based on distance
		rInitial = rgb[0]
		gInitial = rgb[1]
		bInitial = rgb[2]
		rIncrement = (rInitial - rInitial * percentReduction) / 32
		gIncrement = (gInitial - gInitial * percentReduction) / 32
		bIncrement = (bInitial - bInitial * percentReduction) / 32
